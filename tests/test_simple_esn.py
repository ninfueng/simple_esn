...
def test_SimpleESN_shape():
    ...
    assert echoes.shape == (n_samples, n_readout)
...
def test_SimpleESN_discard():
    ...
    echoes = esn.fit_transform(X)
    assert echoes.shape == (n_samples - discard_steps, n_readout)
...
def test_SimpleESN_incorrect_readout():
    ...
    echoes = esn.fit_transform(X)
    assert echoes.shape == (n_samples, n_components)
...
def test_SimpleESN_fit():
    esn = SimpleESN(n_readout=n_readout)
    esn = esn.fit(X)
    assert esn.weights_ is not None
    assert esn.input_weights_ is not None
    assert esn.readout_idx_ is not None
